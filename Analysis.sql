WITH recency_cte AS (
    SELECT c.customer_unique_id,
        DATEDIFF((SELECT MAX(order_purchase_timestamp) FROM olist_orders_clean), MAX(o.order_purchase_timestamp)) AS recency_days
    FROM olist_orders_clean o JOIN olist_customers c ON o.customer_id = c.customer_id
...
rfm_base AS (
    SELECT r.customer_unique_id, r.recency_days, f.frequency, m.monetary
    FROM recency_cte r
